...
const RankingWidget = () => (
  <div className="bg-[#1e1e34] rounded-2xl border border-white/10 overflow-hidden">
    <div className="flex items-center justify-between px-4 py-3 border-b border-white/10">
      ...
      <span className="text-xs font-bold text-yellow-400 cursor-pointer">Ver todo ›</span>
    </div>
    {[
...
export default function Home() {
  return (
    <div className="min-h-screen bg-[#111120] text-white">
      ...
      {/* PAGE */}
      <div className="max-w-screen-2xl mx-auto px-6 py-4 grid grid-cols-1 lg:grid-cols-[1fr_300px] gap-6">

        {/* MAIN */}
        <main>
          {/* HERO */}
          <div className="relative rounded-2xl overflow-hidden h-[240px] mb-6 border border-orange-500/15 bg-gradient-to-br from-[#1a0020] via-[#2d0015] to-[#1a1000]">
            ...
            <div className="absolute left-8 top-1/2 -translate-y-1/2 z-10">
              ...
              <Link href="/juego/test-iq" className="inline-block bg-gradient-to-r from-yellow-400 to-yellow-500 text-black font-black px-8 py-3 rounded-full">
                ¡JUGAR AHORA!
              </Link>
            </div>
          ...
          <section className="mb-6">
            ...
            <div className="flex gap-3 overflow-x-auto pb-2">
              {GAMES.map((game, i) => (
                <Link key={i} href={`/juego/${game.slug}`} className="flex-shrink-0 w-[180px] md:w-[200px] bg-[#1e1e34] rounded-2xl border border-white/10 overflow-hidden hover:-translate-y-1 transition-all group">
                  <div className={`h-[120px] bg-gradient-to-br ${game.color} flex items-center justify-center relative overflow-hidden`}>
                    {game.icon
                    ...
                    <span className="absolute top-2 left-2 bg-yellow-400 text-black text-[11px] font-black px-2 py-0.5 rounded z-10">{game.difficulty}%</span>
                    <span className="absolute top-2 right-2 bg-black/60 text-white text-[10px] font-bold px-2 py-0.5 rounded-full z-10">{game.badge}</span>
                  </div>
                  <div className="p-3">
          ...
          {/* RANKING Y CATEGORIAS — solo en mobile/tablet */}
          ...
          {/* PROMO */}
          <div className="rounded-2xl bg-gradient-to-r from-[#1a1200] to-[#2a1800] border border-yellow-400/20 p-5 flex items-center gap-4 mb-6">
          ...
          <div className="rounded-2xl bg-[#1e1e34] border border-dashed border-white/8 h-[90px] flex items-center justify-center gap-3 text-white/25 text-sm font-bold">
            <span>📢</span><span>Publicidad · Google AdSense 728×90</span>
          </div>
        </main>
        ...
        <aside className="hidden lg:flex flex-col gap-4">
          <RankingWidget />
          <CategoriasWidget />
          <div className="rounded-2xl bg-[#1e1e34] border border-dashed border-white/8 h-[250px] flex flex-col items-center justify-center gap-2 text-white/25 text-sm font-bold">
      ...
      {/* FOOTER */}
      <footer className="border-t border-white/5 bg-[#0e0e1a] py-8 mt-8">
        <div className="max-w-screen-2xl mx-auto px-6">
          <div className="flex flex-wrap gap-6 text-xs text-white/30 font-semibold mb-4">
          ...
          <div className="flex items-center gap-4">
            <div className="bg-white/5 border border-white/8 rounded-lg px-3 py-1.5 text-xs text-white/30 font-bold">G Google AdSense</div>
            <div className="bg-white/5 border border-white/8 rounded-lg px-3 py-1.5 text-xs text-white/30 font-bold">e ezoic</div>
          </div>
        </div>
      </footer>
